...
async function analyzeFile(file, slot = 1) {
  try {
    if (file.type.startsWith('image/')) {
      const se = await checkWithSightengine(file);
      const { score, label, tone, details } = sightengineToVerdict(se);
      const percent = Math.round(score * 100);
      const verdictText = `AI-generated probability: ${percent}% — ${label}`;
      updateMiniVerdict(slot, tone, verdictText);
      showResult(tone, verdictText, details);
    } else if (file.type.startsWith('video/')) {
      // No Sightengine video model in this demo; show unsupported
      showResult('warn', 'Video analysis not supported here', 'Please upload an image for Sightengine analysis.');
    } else {
...
async function checkWithSightengine(file) {
  ...
  if (!resp.ok) throw new Error('Sightengine request failed');
